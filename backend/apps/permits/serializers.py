        ...
        if origins_data is not None:
            ...
            for origin_data in origins_data:
                ...
                if origin_id:
                    ...
                    origin_item = TransportOrigin.objects.get(
                        id=origin_id, application=instance
                    )
                    origin_item.barangay = origin_data.get(
                        "barangay", origin_item.barangay
                    )
                    origin_item.number_of_pigs = origin_data.get(
                        "number_of_pigs", origin_item.number_of_pigs
                    )
                    origin_item.save()
                    keep_origins.append(origin_item.id)
                else:
                    # Create new origin
                    new_origin = TransportOrigin.objects.create(
                        application=instance, **origin_data
                    )
...
class OPVValidationDetailSerializer(serializers.ModelSerializer):
    ...
    opv_staff_username = serializers.CharField(
        source="opv_staff.username", read_only=True, default=None
    )
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    ...
    def validate_veterinary_health_certificate(self, value):
        ...
        if value and value.size > limit:
            raise ValidationError(
                "Veterinary Health Certificate size cannot exceed 30MB."
            )
        return value
