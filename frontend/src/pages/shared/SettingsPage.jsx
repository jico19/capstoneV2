 ...
 * Adapts for all roles (Farmer, Agri, Opv, Inspector).
    ...
    return (
        <div className="p-4 md:p-8 space-y-10 bg-stone-50/50 min-h-full">
            <header className="border-b border-stone-200 pb-8">
                ...
                <h1 className="text-3xl font-black text-stone-800 uppercase tracking-tighter">Your Profile & Settings</h1>
            </header>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                ...
                <section className="bg-white border border-stone-200 rounded-none overflow-hidden">
                    <div className="px-6 py-4 border-b border-stone-200 bg-stone-50/50 flex items-center gap-2">
                        ...
                        <h2 className="text-[10px] font-black text-stone-600 uppercase tracking-widest">Personal Info</h2>
                    </div>
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase tracking-widest text-stone-600">First Name</label>
                            <input {...register("first_name")} className="w-full h-10 px-3 bg-white border border-stone-200 rounded-none text-sm focus:outline-none focus:border-green-700" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase tracking-widest text-stone-600">Last Name</label>
                            <input {...register("last_name")} className="w-full h-10 px-3 bg-white border border-stone-200 rounded-none text-sm focus:outline-none focus:border-green-700" />
                        </div>
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    {...register("phone_no", { pattern: { value: /^(?:\+639|639|09)\d{9}$/, message: "Invalid format" } })}
                                    placeholder="09..."
                ...
                {/* 2. Location (Optional by role) */}
                {isFarmer && (
                    <section className="bg-white border border-stone-200 rounded-none overflow-hidden">
                        <div className="px-6 py-4 border-b border-stone-200 bg-stone-50/50 flex items-center gap-2">
                            <MapPin size={16} className="text-stone-400" />
                            <h2 className="text-[10px] font-black text-stone-600 uppercase tracking-widest">Location</h2>
                        </div>
                        <div className="p-6 space-y-6">
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase tracking-widest text-stone-600">Barangay</label>
                                <select {...register("barangay")} className="w-full h-10 px-3 bg-white border border-stone-200 rounded-none text-sm focus:outline-none focus:border-green-700 appearance-none">
                                    <option value="">Select Barangay</option>
                                    {barangays?.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                                </select>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase tracking-widest text-stone-600">Address Details</label>
                                <input {...register("address")} className="w-full h-10 px-3 bg-white border border-stone-200 rounded-none text-sm focus:outline-none focus:border-green-700" />
                            </div>
                        </div>
                    </section>
                )}
                    ...
                    <section className="bg-white border border-stone-200 rounded-none overflow-hidden">
                        ...
                        <div className="p-6 flex items-center justify-between">
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] font-medium text-stone-500">Receive alerts via phone</p>
                            </div>
                            <input type="checkbox" {...register("receive_sms")} className="checkbox checkbox-success rounded-none border-stone-300" />
                        </div>
                    </section>
                )}

                <div className="flex justify-end pt-4">
                    <button type="submit" disabled={isUpdating} className="bg-green-700 hover:bg-green-600 text-white px-10 py-4 text-xs font-black uppercase tracking-widest rounded-none transition-colors flex items-center gap-2">
